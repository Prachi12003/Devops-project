...
const Storage = {
    ...
    removeFromCart: async (itemId) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/cart/${user.id}/${itemId}`, {
                method: "DELETE"
            });
    ...
    getOrder: async (orderId) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/orders/order/${orderId}`);
            if (response.ok) {
    ...
    getRestaurants: async () => {
        ...
        if (customRestaurants.length === 0) {
            ...
            console.error('2. Is the backend server running on port 3000?');
    ...
    getMenuItems: async (restaurantId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/menu/restaurant/${restaurantId}`);
